    ...
    public function getTabs(): array
    {
        return [
            ...
            'admins' => Tab::make('Admins')
                ->modifyQueryUsing(fn (User $query): Builder => $query->isAdmin())
                ->badge(fn () => User::isAdmin()->count()),
            'students' => Tab::make('Students')
                ->modifyQueryUsing(fn (User $query): Builder => $query->notAdmin())
                ->badge(fn () => User::notAdmin()->count()),
            'active' => Tab::make('Active Learners')
                ->modifyQueryUsing(fn (User $query) => $query->has('enrollments'))
